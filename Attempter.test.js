...
//
// Propagating errors through the Promise chain is currently broken, but someone
// in the future can deal with that.
...
.then(function(numberDeleted){
  ...
  for (var i = 0; i < (numberOfWorkUnitsToTest4/2); i++){
    // in redis-land, scores go first
    redisArgs.push(16751491200); // timestamp in 2500; should never get popped
      ...
      if (tickCount > (numberOfWorkUnitsToTest4/2)) {
        if (mockService.openedRequestsCount === (numberOfWorkUnitsToTest4/2)) {
          ...
          if (futureDetected === true){
            reject("The attempter appears to have ordered units in the future.");
          } else {
